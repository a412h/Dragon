...
public:
    ...
    double kappa_reference = 1.866666666666666e-2;
    bool use_navier_stokes = false;  // Default to Euler
    ...
    void read_parameters(const std::string& filename) {
        ...
        if (use_navier_stokes) {
            ...
            // kappa = mu * c_p / Pr = mu * gamma / ((gamma - 1) * Pr)
            kappa_reference = mu_reference * gamma / ((gamma - 1.0) * prandtl_number);
        }
        ...
        if (use_navier_stokes) {
            ...
            std::cout << "  Thermal conductivity: " << kappa_reference << std::endl;
        }
    }
};
