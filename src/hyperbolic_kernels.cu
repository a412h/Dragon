...
// ============================================================================
// Optimized atomic functions
// ============================================================================
__device__ __forceinline__ void atomicMin_(float* addr, float value) {
    atomicMin(reinterpret_cast<int*>(addr), __float_as_int(value));
}

__device__ __forceinline__ void atomicMin_(double* addr, double value) {
    ...
    do {
        expected = old;
        old = atomicCAS(addr_as_ull, expected,
...
// ============================================================================
// KERNEL 3b: Compute diagonal and tau (designed for shared memory)
// ============================================================================
template<int dim, typename Number>
    ...
    int n_dofs)
{
    ...
    // Load U_i_new and F_iH into registers
    Number U_i_new_array[dim + 2];
