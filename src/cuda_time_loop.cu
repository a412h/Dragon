...
// ============================================================================
// Helper functions for SoA memory operations
// ============================================================================
...
public:
    StageExecutor(
        ...
        : d_U(_d_U), d_temp_0(_d_temp_0), d_temp_1(_d_temp_1), d_temp_2(_d_temp_2),
          ...
          n_dofs(_n_dofs), nnz(_nnz), offline_data(_offline_data), stream(_stream)
    {
        ...
        lowOrderConfig.threadsPerBlock = 128;  // Reduced for high register usage
        ...
        // Set L1 cache preference for memory-bound kernels
    ...
    VTUOutput<dim>* output_handler)
{
    ...
    StageExecutor<dim, Number_cu> hyperbolic_executor(
        ...
        Number_cu(1.0), n_dofs, nnz_mij, offline_data, compute_stream);
    ...
    for (int warmup = 0; warmup < 10; ++warmup) {
        Number_cu tau = hyperbolic_executor.execute_timestep(config.final_time);
        t += tau;
        step++;
    ...
    while (t < config.final_time) {
        ...
        // Execute batch of timesteps
        Number_cu batch_dt = 0;
        for (int batch_step = 0; batch_step < n_steps_per_batch; ++batch_step) {
            Number_cu tau = hyperbolic_executor.execute_timestep(tau_max_remaining);
            batch_dt += tau;
